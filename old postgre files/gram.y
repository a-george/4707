...
%type <node>	ignore_clause select_ignore_value
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	LEAST LEFT LEVEL LIKE LIMIT LIMITTWO LISTEN LOAD LOCAL LOCALTIME LOCALTIMESTAMP
	LOCATION LOCK_P
	...
	OBJECT_P OF OFF OFFSET OFFSETTWO OIDS ON ONLY OPERATOR OPTION OPTIONS OR
	...
	SAVEPOINT SCHEMA SCROLL SEARCH SECOND_P SECURITY SEQUENCE SEQUENCES
			...
			group_clause having_clause window_clause
				{
					...
					/*
					 * TODO: edit the SelectStmt struct in parsenodes.h to include an ignoreClause
					 */
					SelectStmt *n = makeNode(Stmt); 
					n->distinctClause = $2;
					n->targetList = $3;
					n->intoClause = $4;
					n->fromClause = $5;
...
/* 
 * The IGNORE clause has a very similar grammar (?) to the OFFSET clause
 * IGNORE is essentially OFFSET
 */
...
/* This portion allows for the IGNORE clause to be optional
 * within a SELECT query. Else, IGNORE would need to be added
 * to every single SELECT query.
 */
...
/* The value to be read by IGNORE */
