...
/* Build an Ignore plan node. Based very heavily upon OFFSET code, as the situation
 * is the same for both OFFSET and IGNORE.
 */
Ignore *
make_ignore(Plan *lefttree, Node *ignoreClause,
            int64 ignore_est)
{
    ...
    /*
     * Adjust the output rows count and costs according to the k value of IGNORE.
     * As per LIMIT/OFFSET, this is necessary because: "This is only a cosmetic 
     * issue if we are at top level, but if we are
     * building a subquery then it's important to report correct info to the
     * outer planner.
     *
     * When the k value of IGNORE couldn't be estimated, use 10% of the
     * estimated number of rows emitted from the subplan.
     */
    if (ignore_est != 0)
    {
        ...
        if (ignore_est > 0)
            ignore_rows = (double) ignore_est;
        else
            ignore_rows = clamp_row_est(lefttree->plan_rows * 0.10);
        if (ignore_rows > plan->plan_rows)
            ignore_rows = plan->plan_rows;
        if (plan->plan_rows > 0)
            plan->startup_cost +=
            (plan->total_cost - plan->startup_cost)
            * ignore_rows / plan->plan_rows;
        plan->plan_rows -= ignore_rows;
        if (plan->plan_rows < 1)
            plan->plan_rows = 1;
    }
    ...
    node->ignoreClause = ignoreClause;
