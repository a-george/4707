                 ...
                 PlannerInfo **subroot)
{
    ...
    parse->limitCount = preprocess_expression(root, parse->limitCount,
                                              EXPRKIND_LIMIT);
    /* Use EXPRKIND_LIMIT for simplicity */
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
    ...
    int64		offset_est = 0;
    int64		count_est = 0;
    int64       ignore_est = 0; // Estimate of the effects of IGNORE
    double		limit_tuples = -1.0;
    double      ignore_tuples = -1.0; // Number of tuples to be ignored
    Plan	   *result_plan;
    ...
    /* If we have an ignoreClause, we update tuple_fraction to the number of tuples that 
     * should be skipped by IGNORE. Like the case for LIMIT/OFFSET, if we have a known 
     * value for IGNORE, then we can estimate the effect.
     */
    ...
    /* 
     * If there is an IGNORE clause, add an IGNORE node.
     */
    if (parse->ignoreClause)
    {
...
/*
 * preprocess_ignoreClause - do prep. work on the IGNORE clause
 *
                        ...
                        int64 *ignore_est)
{
    ...
    if (est && IsA(est, Const))
    {
        if (((Const *) est)->constisnull)
        {
            /* Treat NULL as no IGNORE; i.e. treat it as an IGNORE with k value 0; the executor will too */
            *ignore_est = 0;
        }
        else
        {
            /* In the case where IGNORE is k < 0, treat this as k = 0. */
        ...
        *ignore_est = -1;	/* can't estimate k for IGNORE */
    
    if (*ignore_est != 0 && tuple_fraction > 0.0)
    {
        /*
         * An IGNORE clause. We need to fetch more tuples if tuple_fraction > 0.
         * Default value of ignore_fraction is 10%.
         */
        if (*ignore_est < 0)
        ...
        /*
         * Similar case as OFFSET:
         * "If we have absolute counts from both caller and IGNORE, add them
         * together; likewise if they are both fractional.	If one is
         * fractional and the other absolute, we want to take the larger, and
         * we heuristically assume that's the fractional one."
         */
        ...
        else
        {
            if (ignore_fraction >= 1.0)
            {
                /* caller fractional, ignore absolute; use caller's value */
            }
            else
            {
                /* both fractional, so add them together */
